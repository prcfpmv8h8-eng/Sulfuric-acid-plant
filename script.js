...
const backToFacilities = document.getElementById("backToFacilities");
...
function sectionLabel(value) {
  return String(value || "").trim() || "ללא קטגוריה";
}

function categoryVisual(section) {
  const value = String(section || "").trim();
  if (value.includes("קירור")) return { icon: "💧", tone: "cooling" };
  if (value.includes("בור")) return { icon: "▦", tone: "pits" };
  if (value.includes("מים")) return { icon: "≈", tone: "water" };
  if (value.includes("חומצה")) return { icon: "◈", tone: "acid" };
  return { icon: "▦", tone: "default" };
}

function renderCategories(facility) {
  const sections = [...new Set(
    equipment
      .filter(item => item.facility === facility)
      .map(item => sectionLabel(item.section))
  )].sort((a, b) => a.localeCompare(b, "he"));

  categoriesTitle.textContent = `קטגוריות במתקן ${facility}`;
  categoryGrid.innerHTML = "";
  categoryEmpty.hidden = sections.length > 0;

  sections.forEach(section => {
    const count = equipment.filter(item =>
      item.facility === facility && sectionLabel(item.section) === section
    ).length;

    const visual = categoryVisual(section);
    const button = document.createElement("button");
    button.className = `category-card category-${visual.tone}`;
    button.type = "button";
    button.innerHTML = `
      <span class="category-icon" aria-hidden="true">${visual.icon}</span>
      <span class="category-content">
        <strong>${section}</strong>
        <small>${count} יחידות ציוד</small>
        <em>כניסה לקטגוריה</em>
      </span>
...
function applyFilters() {
  ...
  if (activeSection) filtered = filtered.filter(item => sectionLabel(item.section) === activeSection);
  if (query) filtered = filtered.filter(item => searchableText(item).includes(query));

  if (!query && (!activeFacility || !activeSection)) {
    resultsGrid.innerHTML = "";
  ...
  const title = query
    ...
    : `${activeSection} — מתקן ${activeFacility}`;
  const filters = [];
  if (activeFacility) filters.push(`מתקן ${activeFacility}`);
  if (activeSection) filters.push(activeSection);
  searchMessage.textContent = filters.length ? `מסנן פעיל: ${filters.join(" · ")}` : "";
...
facilityButtons.forEach(button => {
  button.addEventListener("click", () => {
    ...
    activeSection = null;
    searchInput.value = "";
    renderCategories(activeFacility);
    applyFilters();
    categoriesSection.scrollIntoView({ behavior: "smooth", block: "start" });
  });
});

backToFacilities.addEventListener("click", () => {
  activeFacility = null;
  activeSection = null;
  categoriesSection.hidden = true;
  applyFilters();
